...
//	@x-api-type		{"sdk":"true"}
func GetGroupStream(c *gin.Context) {
	...
	processor, err := producer.NewGroupStreamProcessor(groupID)
	if err != nil {
	...
	// Read historical events (traces that already completed before SSE connection)
	logEntry.Info("Reading historical group stream events")
	historical, err := producer.ReadGroupStreamMessages(ctx, streamKey, req.LastID, 100, 0)
	if err != nil {
	...
	if len(historical) > 0 {
		...
		if completed {
			logEntry.Info("Group completed during historical events, closing stream")
			return
		}
	...
	logEntry.Infof("Switching to real-time group event monitoring from ID: %s", req.LastID)
	for {
		select {
		case <-ctx.Done():
		...
		default:
			newMessages, err := producer.ReadGroupStreamMessages(ctx, streamKey, req.LastID, 10, time.Second)
			if err != nil {
				if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
					logEntry.Infof("Context done while reading group stream: %v", err)
					return
				}
			...
			req.LastID = lastID
			if completed {
				logEntry.Info("All traces in group completed, closing stream")
...
// sendGroupSSEEvents processes group stream messages and sends them as SSE events
func sendGroupSSEEvents(c *gin.Context, processor *producer.GroupStreamProcessor, streams []redis.XStream) (string, bool, error) {
