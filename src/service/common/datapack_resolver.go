...
func ExtractDatapacks(db *gorm.DB, datapackName *string, datasetRef *dto.DatasetRef, userID int, taskType consts.TaskType) ([]model.FaultInjection, *int, error) {
	...
	if datasetRef != nil {
		datasetVersionResults, err := MapRefsToDatasetVersionsWithDB(db, []*dto.DatasetRef{datasetRef}, userID)
		if err != nil {
