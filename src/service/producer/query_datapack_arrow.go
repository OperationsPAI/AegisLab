...
// QueryDatapackFileContent reads a parquet file and streams it as an Arrow IPC stream.
...
// a SELECT that casts UINT64/UHUGEINT columns to signed BIGINT so Arrow IPC consumers
// that do not support unsigned 64-bit integers can still parse the stream.
