...
func (a *InjectionAnalyzer) Analyze(injections []database.FaultInjection) (map[string]dto.InjectionStats, error) {
...
func (a *InjectionAnalyzer) getNode(item *database.FaultInjection) (*chaos.Node, string, error) {
	nsPrefix, err := extractNamespacePrefix(item.Name)
	if err != nil {
...
// extractNamespacePrefix extracts the namespace prefix from the injection name
...
// extractPairService extracts the source and target services from a pair string
...
// recursiveToGetRangeNum traverses the chaos node tree to calculate the total range number
...
// recursiveToGetCoveredMap traverses the chaos node tree to get the covered map
