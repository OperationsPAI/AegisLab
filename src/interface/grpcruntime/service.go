...
func listNamespaceLocks(ctx context.Context, redis *redisinfra.Gateway) (map[string]map[string]any, error) {
...
func listQueuedTasks(ctx context.Context, redis *redisinfra.Gateway) (map[string]any, error) {
...
func decodeQueuedTasks(items []string) ([]taskmodule.TaskResp, error) {
	result := make([]taskmodule.TaskResp, 0, len(items))
	for _, item := range items {
		var task dto.UnifiedTask
		if err := json.Unmarshal([]byte(item), &task); err != nil {
			return nil, err
		}
		result = append(result, taskmodule.TaskResp{
			ID:          task.TaskID,
