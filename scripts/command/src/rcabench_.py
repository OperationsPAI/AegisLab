...
def execute_release_workflow(env: ENV, k8s_manager: KubernetesManager):
    ...
    # Ensure setValues exists in helm release config
    helm_release = skaffold_config["deploy"]["helm"]["releases"][0]
    if "setValues" not in helm_release:
        helm_release["setValues"] = {}

    original_image_repo = helm_release["setValues"].get("image.repository", "")
    ...
    if original_image_repo:
        skaffold_config["deploy"]["helm"]["releases"][0]["setValues"][
            "image.repository"
        ] = original_image_repo
    else:
        # Remove setValues if it was empty originally
        helm_release = skaffold_config["deploy"]["helm"]["releases"][0]
        if "setValues" in helm_release and not helm_release["setValues"]:
            del helm_release["setValues"]
