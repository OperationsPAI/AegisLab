...
class MetricParser(Parser):
    ...
    async def parse(self, pool: Pool):
        tasks = [
            self._process_metric_data(pool, normal=normal) for normal in [True, False]
        ]
    ...
    async def _process_metric_data(self, pool: Pool, normal: bool):
        file_path = (
            self.normal_group_metric_path if normal else self.abnormal_group_metric_path
        )
        df = pd.read_csv(file_path, engine="pyarrow").dropna(subset=["k8s_pod_name"])
        output_dir = (
            self.base_dir / ("normal" if normal else "abnormal") / "processed_metrics"
        )
...
async def batch_process(batch: pd.DataFrame, pod):
    ...
    pivoted = batch.pivot_table(
        index="TimeUnix", columns="MetricName", values="Value", aggfunc="first"
    )
...
async def process_pod(
    pod, group: pd.DataFrame, output_dir: Path, pool: Pool, batch_size=10000
):
    if batch_size >= len(group) * 0.5:
        ...
        processed_batches = []
        batches = [
            group.iloc[start : start + batch_size].copy()
            for start in range(0, len(group), batch_size)
        ]
        processed_batches = await pool.starmap(
            batch_process, zip(batches, [pod] * len(batches))
        )
