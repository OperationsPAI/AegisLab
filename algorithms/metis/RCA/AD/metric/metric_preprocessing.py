...
def devide_by_pod_name(file_path):
    ...
    for pod_name, logs in pod_metrics.items():
        if "redis-cart" in pod_name or "loadgenerator" in pod_name or "mysql" in pod_name:
            continue
        ...
        with open(os.path.join(processed_file_path, file_name), mode='w', encoding='utf-8') as file:
