...
async def detect(base_dir, data):
    ...
    case_name = "_".join(base_dir.parts[-2:])  # Extract the case name from the base directory
...
def detect_anomalies(normal_df, abnormal_df, k=3):
    ...
    for start_time, group in abnormal_df.groupby("StartTime"):
        ...
        for _, row in group.iterrows():
            ...
            normal_stats = normal_df[(normal_df["ServiceName"] == service_name) & (normal_df["SpanName"] == span_name)]
                ...
                if mean_duration > normal_mean + k * normal_std:
                    if service_name not in anomalies:
                        ...
                        anomalies[service_name]["TimeRanges"] = (
                            ...
                            max(anomalies[service_name]["TimeRanges"][1], start_time + pd.Timedelta(minutes=2)),
                        )
