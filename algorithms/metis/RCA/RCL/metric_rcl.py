...
def calculate_overlap(normal_path, abnormal_path, output_path, k = 0.8):
    ...
    overlap_results = pd.DataFrame(columns=['service_name', 'metric_name', 'anomaly_score', 'overlap_percentage',
                                            'avg_change', 'abnormal_max', 'abnormal_min', 'abnormal_average',
                                            'normal_max', 'normal_min', 'normal_average'])
        ...
        if os.path.exists(abnormal_file):
            ...
            for i, row in normal_data.iterrows():
                ...
                if not abnormal_row.empty:
                    ...
                    avg_change = abs(abnormal_average - normal_average) / normal_average * 100 if normal_average != 0 else 0
                    ...
                    overlap_results = pd.concat([overlap_results, result], ignore_index=True)
...
def process_and_output_metric_events(file_path):
    ...
    for index, row in df_sorted.iterrows():
        ...
        print(f"  - overlap_percentage_between_normal_and_abnormal: {row['overlap_percentage']}")
        print(f"  - [abnormal_min, abnormal_max]: [{row['abnormal_min']},{row['abnormal_max']}]")
        print(f"  - [normal_min, normal_max]: [{row['normal_min']},{row['normal_max']}]")
