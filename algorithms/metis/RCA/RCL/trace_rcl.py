...
def read_and_merge_data(normal_path, abnormal_path):
    ...
    merged_df = pd.merge(
        normal_df,
        abnormal_df,
        on=["ServiceName", "SpanName"],
        suffixes=("_normal", "_abnormal"),
    )
    return merged_df
...
def calculate_changes(merged_df):
    merged_df["ChangePercentage"] = (
        (merged_df["MeanDuration_abnormal"] - merged_df["MeanDuration_normal"])
        / merged_df["MeanDuration_normal"]
    ) * 100
...
def sort_and_filter_data(merged_df):
    ...
    aggregated_df = (
        merged_df.groupby(["ServiceName", "SpanName", "ParentServiceName_abnormal"])
        .agg(
    ...
    aggregated_df = aggregated_df.sort_values(
        by="ChangePercentage", key=abs, ascending=False
    )
    ...
    filtered_df.loc[:, "WeightedChange"] = filtered_df["ChangePercentage"] * (
        1 / (filtered_df.index + 1)
    )
    ...
    filtered_df.loc[:, "anomaly_score"] = filtered_df.apply(
        lambda row: row["WeightedChange"]
        * (service_span_count[row["ServiceName"]] / total_spans),
        axis=1,
    )
        ...
        trace_events.append(
            {
                ...
                "span_num": row["SpanNum"],
                "service": row["ServiceName"],
                "parent_service": row["ParentServiceName_abnormal"],
                "normal_duration_per_min": round(
                    row["MeanDuration_normal"] / 60000000, 3
                ),
