...
def weighted_change(filtered_df):
    ...
    filtered_df.loc[:, "ProportionalWeightedChange"] = filtered_df.apply(
        ...
        * (span_count[row["ServiceName"]] / total_spans),
        axis=1,
    )
    result = (
        filtered_df.groupby("ServiceName")["ProportionalWeightedChange"]
        .sum()
        .reset_index()
    )
...
def print_top_spans(file_path):
    ...
    for index, row in df.head(5).iterrows():
        ...
        print(
            f"    - normal_duration(per min): {row['MeanDuration_normal'] / 60000000:.2f}s"
        )
        print(
            f"    - observed_duration: {row['MeanDuration_abnormal'] / 60000000:.2f}s"
        )
