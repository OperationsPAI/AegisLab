...
def process_log_files(input_folder_path, output_folder_path):
                ...
                for row in reader:
                    ...
                    try:
                        timestamp = datetime.strptime(
                            timestamp_str[:26], "%Y-%m-%d %H:%M:%S.%f"
                        )
                    except ValueError:
                        timestamp = datetime.strptime(
    ...
    # 计算时间跨度
    total_minutes = (
        (max(time_stamps) - min(time_stamps)).total_seconds() / 60 if time_stamps else 1
    )

    # 准备输出数据
    output_data = []
    for service_name in error_template_counts.keys():
        for temp_id in set(error_template_counts[service_name].keys()).union(
            warn_template_counts[service_name].keys()
        ):
            ...
            most_common_body = max(
                set(log_bodies[service_name][temp_id]),
                key=log_bodies[service_name][temp_id].count,
            )
            output_data.append(
                [
                    service_name,
...
def process_and_output_log_data(csv_file_path):
    ...
    if "Normal Rate/Min" not in df.columns:
        df["Normal Rate/Min"] = [0] * len(
            df
        )  # 示例中的预设值，具体实现需根据实际情况来计算或修改
    ...
    for index, row in df.head(5).iterrows():
        ...
        output.append(
            "- normal_frequence (per min): {:.2f}".format(row["Normal Rate/Min"])
        )
    ...
    # 输出保存到CSV文件
    sorted_scores.to_csv(
        os.path.join(os.path.dirname(csv_file_path), "log_service_scores.csv"),
        header=["Score"],
    )
