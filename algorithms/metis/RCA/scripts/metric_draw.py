...
for filename in os.listdir(abnormal_folder):
    if filename.endswith(".csv"):
        ...
        # 检查两个数据集中是否包含所有所需的列
        available_columns_abnormal = [
            col for col in metric_columns if col in abnormal_data.columns
        ]
        available_columns_normal = [
            col for col in metric_columns if col in normal_data.columns
        ]

        # 保证两个数据集的度量列一致
        available_columns = list(
            set(available_columns_abnormal) & set(available_columns_normal)
        )
        ...
        combined_data = pd.concat(
            [abnormal_data[available_columns], normal_data[available_columns]], axis=0
        )
        ...
        for metric in available_columns:
            ...
            # 绘制 abnormal 数据
            plt.plot(
                abnormal_data["TimeUnix"].values,
                abnormal_data[metric].to_numpy(),
                label=f"abnormal_{metric}",
                color=color,
                linestyle="-",
                alpha=0.7,
            )
            ...
            # 绘制 normal 数据
            plt.plot(
                normal_data["TimeUnix"].values,
                normal_data[metric].to_numpy(),
                label=f"normal_{metric}",
                color=color,
                linestyle="--",
                alpha=0.7,
            )
