...
class Task:
    ...
    async def get_stream(self, task_ids: List[UUID]) -> None:
        urls = [
            f"{self.url_prefix}{self.URL_ENDPOINTS['get_stream'].format(task_id=task_id)}"
            for task_id in task_ids
        ]

        await self.stream.start_multiple_stream(urls, client_ids=task_ids)

    @async_validate_request_response(response_model=StreamResult)
    async def get_stream_batch(
        self, task_ids: List[UUID], timeout: Optional[float] = None
    ) -> StreamResult:
        await self.get_stream(task_ids)
