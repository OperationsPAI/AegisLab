...
class Trace:
    ...
    async def get_stream_single(
        ...
        url = f"{self.url_prefix}{self.URL_ENDPOINTS['get_stream'].format(trace_id=trace_id)}"
    ...
    async def get_stream_batch(
        self, trace_ids: List[UUID], client_timeout: float
    ) -> asyncio.Queue[QueueItem]:
        urls = [
            f"{self.url_prefix}{self.URL_ENDPOINTS['get_stream'].format(trace_id=trace_id)}"
